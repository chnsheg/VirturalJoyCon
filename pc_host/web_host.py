...
def build_runtime_notes(bind_host: str, http_port: int, udp_port: int, access_urls: Iterable[str]) -> List[str]:
    ...
    notes = [
        ...
        "[GamepadHost] Public control path prefers WebRTC DataChannel; WebSocket stays warm and HTTP is last resort",
        "[GamepadHost] Requested FPS may be clamped to the source refresh rate or runtime caps",
        "[GamepadHost] Requested/effective stream profile is exposed by stream_gateway.py /api/stream/settings and checkable in .runtime/stream_settings.active.json",
